...
import { setupCanvasMocks, resetCanvasMocks } from '../setup/mocks/canvas'
...
describe('Phase 4: Easter Egg Discovery System', () => {
  ...
  describe('Pattern Recognition Accuracy', () => {
    ...
    test('should accurately detect keyboard sequence patterns', async () => {
      ...
      await act(async () => {
        result.current.actions.loadModule = vi.fn().mockImplementation(() => {
      ...
      await act(async () => {
        for (const key of sequence) {
          ...
          await testUtils.waitForAnimationFrames(2);
        }
      });
    ...
    test('should detect complex mouse gesture patterns', async () => {
      ...
      await act(async () => {
        for (let i = 0; i <= points; i++) {
          ...
          await testUtils.waitForAnimationFrames(1); // Wait for frame processing
        }
      });
    ...
    test('should recognize scroll-based patterns with tolerance', async () => {
      ...
      await act(async () => {
        for (const scrollAmount of scrollPattern) {
          ...
          // Wait for scroll event processing with timing tolerance
          await testUtils.waitForAnimationFrames(5); // Equivalent to ~83ms at 60fps
        }
      });
  ...
  describe('Achievement Tracking', () => {
    ...
    test('should persist achievement progress across sessions', async () => {
      ...
      await act(async () => {
        await testUtils.waitForAnimationFrames(3); // Allow for preferences loading
      });
